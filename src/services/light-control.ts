...
export class LightControlService {
  ...
  //         .string()
  //         .describe(
  //           "灯光设备的实体ID，如 'light.living_room' 或多个设备用逗号分隔"
  //         ),
  //       action: z
  //         .enum([
  //           "turn_on",
   ...
   */
  @Tool("控制灯光设备 - 支持通过名称控制灯光设备")
  async controlLightByName(
    ...
    if (entities.length <= 0) return "没有找到可以控制的设备";
  ...
  //     // 筛选灯光设备
  //     const lightStates = allStates.filter((state: HassState) =>
  ...
  //     if (params.room) {
  //       filteredStates = lightStates.filter((state: HassState) =>
  //         this.matchesRoom(state, params.room!)
  //       );
  //     }
  ...
  //     (params.brightness < 1 || params.brightness > 100)
  //   ) {
  //     throw new LightControlError(
  //       "亮度值必须在 1-100 之间",
  //       LightControlErrorCodes.INVALID_BRIGHTNESS,
  ...
  //     (params.temperature < 2700 || params.temperature > 6500)
  //   ) {
  //     throw new LightControlError(
  ...
  //  * 执行灯光控制动作
  //  */
  // private async executeLightAction(
  //   entityIds: string[],
  //   params: LightControlParams
  // ): Promise<HassState[]> {
  ...
  //  */
  // private convertToHassServiceParams(
  //   params: LightControlParams
  // ): HassLightServiceParams {
  ...
  //         serviceParams.brightness = Math.round(
  //           (params.brightness / 100) * 255
  //         );
  //       }
  //       break;
  ...
  //           serviceParams.brightness = Math.round(
  //             (sceneConfig.brightness / 100) * 255
  //           );
  //         }
  ...
  //           serviceParams.color_temp = Math.round(
  //             1000000 / sceneConfig.temperature
  //           );
  //         }
  ...
  //  */
  // private convertToLightDeviceInfo(
  //   state: HassState,
  ...
  //       deviceInfo.brightness = Math.round(
  //         ((state.attributes["brightness"] as number) / 255) * 100
  //       );
  //     }
  ...
  //       // 将 mired 转换为 Kelvin
  //       deviceInfo.temperature = Math.round(
  //         1000000 / (state.attributes["color_temp"] as number)
  //       );
  //     }
