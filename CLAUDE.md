...
**重要：每次修改代码后必须执行完整检查，避免CI/CD失败**

在提交代码或创建PR之前，必须执行以下命令确保代码质量：

```bash
# 执行所有类型检查、代码规范检查和拼写检查
nr check:all
...
#### 检查命令说明
- **`nr check:all`**: 包含以下检查
  - `pnpm check` - Biome代码规范和格式检查
  - `pnpm type:check` - TypeScript类型检查
  - `pnpm spell:check` - 拼写检查

- **`nr test`**: 运行所有测试用例，确保功能正常
...
#### 常见问题排查
- **类型检查失败**: 检查是否有`any`类型使用或类型定义不完整
- **代码规范失败**: 运行 `nr check:fix` 自动修复
- **拼写检查失败**: 检查专业术语或添加到自定义词典
- **测试失败**: 使用 `/fix-test` 命令分析和修复失败的测试用例

通过严格执行这个流程，可以确保代码质量并避免CI/CD流水线失败。
...
- **`ci-validator`** - CI检查验证和质量保障
  - 自动执行完整的代码质量检查流程
  - 智能诊断检查失败原因并提供修复建议
  - 确保代码符合CI标准和最佳实践
  - 验证类型安全、代码规范、拼写检查和测试覆盖率
