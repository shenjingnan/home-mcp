...
我是 API 文档自动生成技能，专门从 Home-MCP 项目的源代码中提取 API 信息，生成符合项目标准的 MDX 文档。
...
### 2. MDX 文档生成
基于提取的信息生成符合项目标准的 Nextra MDX 文档。
...
function updateNavigation(docs: GeneratedDoc[]): void {
  const metaContent = generateMetaContent(docs);
  writeFileSync('docs/content/mcp-tools/_meta.ts', metaContent);
}
```
...
通过这个技能，可以确保 Home-MCP 项目的 API 文档始终保持最新、准确和高质量，提升开发者体验和项目可维护性。