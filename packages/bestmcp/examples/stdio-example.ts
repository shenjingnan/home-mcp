...
class TextService {
  @Tool("将文本转换为大写")
  async toUpperCase(@Param(z.string(), "输入文本") text: string): Promise<string> {
    return text.toUpperCase();
  }

  @Tool("计算文本长度")
  async calculateLength(@Param(z.string(), "输入文本") text: string): Promise<number> {
    return text.length;
  }

  @Tool("反转文本")
  async reverseText(@Param(z.string(), "输入文本") text: string): Promise<string> {
