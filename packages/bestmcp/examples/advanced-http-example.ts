...
class UserService {
  ...
  @Tool("获取用户信息")
  async getUser(@Param(z.string(), "用户ID") userId: string): Promise<User | null> {
  ...
  @Tool("删除用户")
  async deleteUser(@Param(z.string(), "用户ID") userId: string): Promise<{ success: boolean; message: string }> {
    const user = users.get(userId);
    if (!user) {
...
class PostService {
  ...
  @Tool("获取文章信息")
  async getPost(@Param(z.string(), "文章ID") postId: string): Promise<Post | null> {
  ...
  @Tool("获取作者的文章")
  async getPostsByAuthor(@Param(z.string(), "作者ID") authorId: string): Promise<Post[]> {
    const author = users.get(authorId);
    if (!author) {
      throw new Error(`作者 ${authorId} 不存在`);
    }

    const authorPosts = Array.from(posts.values()).filter((post) => post.authorId === authorId);

    return authorPosts;
  }

  @Tool("删除文章")
  async deletePost(@Param(z.string(), "文章ID") postId: string): Promise<{ success: boolean; message: string }> {
    const post = posts.get(postId);
    if (!post) {
...
async function startAdvancedHTTPServer() {
  try {
    ...
    console.log(`  -H "Content-Type: application/json" \\`);
    console.log(
      `  -d '{"jsonrpc": "2.0", "id": 2, "method": "tools/call", "params": {"name": "createUser", "arguments": {"name": "张三", "email": "zhangsan@example.com"}}}'`,
    );
    console.log("");
    console.log(`# 获取服务器状态`);
    console.log(`curl -X POST http://${host}:${port}/mcp \\`);
    console.log(`  -H "Content-Type: application/json" \\`);
    console.log(
      `  -d '{"jsonrpc": "2.0", "id": 3, "method": "tools/call", "params": {"name": "getServerStatus", "arguments": {}}}'`,
    );
