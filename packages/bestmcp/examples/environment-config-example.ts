...
function parseConfig(): AppConfig {
  ...
  if (config.transport === 'http') {
    config.port = parseInt(env.PORT || '8000');
...
async function main() {
  try {
    ...
    if (config.transport === 'http') {
      ...
      console.log(`  -d '{"jsonrpc": "2.0", "id": 1, "method": "tools/call", "params": {"name": "getConfig", "arguments": {}}}'`);
...
export { AppConfig, parseConfig, validateConfig, LoggingService, MetricsService, ConfigurableService };