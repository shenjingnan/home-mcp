...
describe("双传输层集成测试", () => {
  ...
  describe("传输层切换", () => {
    it("应该能够在 stdio 和 HTTP 传输层之间切换", async () => {
      ...
      const transportManagerSpy = vi.spyOn(mcp['transportManager'], 'startCurrentTransport').mockResolvedValue(undefined);
  ...
  describe("复杂场景集成测试", () => {
    ...
    it("应该处理传输层切换期间的错误", async () => {
      ...
      // 模拟传输层启动失败
      const errorSpy = vi.spyOn(mcp['transportManager'], 'startCurrentTransport')
